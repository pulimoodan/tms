...
export default function WaybillReportPage() {
  ...
  const handleExport = () => {
    ...
    link.setAttribute('download', `waybill-report-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handleExportExcel = () => {
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `waybill-report-${new Date().toISOString().split('T')[0]}.xlsx`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="min-w-0 flex-shrink">
          <PageTitle
            ...
            description="Comprehensive trip and waybill report with detailed information"
            icon={File01Icon}
          />
        </div>
